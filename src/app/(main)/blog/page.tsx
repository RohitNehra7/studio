...
const blogPosts = [
  {
    ...
    excerpt: 'Exploring how eco-friendly materials and design are shaping the future of our cities and homes.',
        ...
        {blogPosts.map(post => (
          <Card key={post.id} className="overflow-hidden h-full flex flex-col group transition-all duration-300 hover:shadow-2xl hover:border-primary">
            ...
            <CardFooter className="p-6 pt-0 flex justify-between items-center">
                <div>
                {post.tags.map(tag => (
                    <Badge key={tag} variant="secondary" className="mr-2 bg-accent/20 text-accent-foreground">{tag}</Badge>
                ))}
                </div>
